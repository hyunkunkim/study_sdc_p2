...
def plot_tracks(fig, ax, ax2, track_list, meas_list, lidar_labels, lidar_labels_valid, 
    ...
    ax.legend(handle_list, label_list, loc='upper left', shadow=True, fontsize='x-large', bbox_to_anchor=(1.05, 1.0))
    plt.tight_layout()
    #plt.pause(0.01)
    plt.show()
    #fig.canvas.draw()
...
def plot_rmse(manager, all_labels, configs_det):
    ...
    if plot_empty: 
        ...
        plt.legend(loc='upper left', shadow=True, bbox_to_anchor=(1.05, 1))
        plt.tight_layout()
        plt.xlabel('time [s]')
        plt.ylabel('RMSE [m]')
        plt.show()
